...
<script>
  ...
  export default {
    ...
    computed: {
      message: function () {
        return (this.status && this.status.trainingDataUploaded) ? [
          `You have selected ${this.status.trainingDataFilename}.`, 'Click to upload a new file.'
        ] : ['Select a file with training data and get going!']
      }
    },
    created() {
      this.getStatus()
